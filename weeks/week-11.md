...
Энергия связи $`B>0`$ — энергия, необходимая для разборки ядра на свободные нуклоны:
...
В ядрах обычно ниже уровень $`j=l+1/2`$, что соответствует $`a<0`$ в этой записи. Чётность однонуклонного состояния равна $`(-1)^l`$; правила для фотонов $`Ek,Mk`$ приведены в [теории недели 9](./week-09.md).
...
**Решение.** Для обоих ядер $`A^{1/3}=3`$, $`(A-2Z)^2=1`$, а спаривание отсутствует, поскольку $`A`$ нечётно. Кремний сильнее расталкивается электрически, поэтому $`B_{\rm Al}>B_{\rm Si}`$:
...
Вторая производная отрицательна, то есть это максимум связи. При $`Z<Z_0`$ выгодно превращение $`n\to p+e^-+\bar\nu_e`$, а при $`Z>Z_0`$ — уменьшение $`Z`$ через $`\beta^+`$ или электронный захват.
