...
**Условие.** Поверхность некоторого тела приготовлена таким образом, что коэффициент поглощения электромагнитного излучения $`A = 1`$ для частот $`\omega\le\omega_0`$ и $`A = 0`$ при $`\omega\gt \omega_0`$. Это тело помещено в вакуум и в отсутствие других источников излучения нагревается за счет внутреннего источника энергии до температуры $`T`$. Определить эту температуру, если известно, что для такого же тела с абсолютно черной поверхностью в тех же условиях равновесная температура $`T^* = 300`$ К. Граничная частота соответствует температуре $`\Theta = \hbar\omega_0/k_{\mathrm{Б}} = 300`$ К.
