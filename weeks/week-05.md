...
Для $`0\lt x\lt l`$ потенциал равен нулю, а вне этого интервала бесконечен. Внутри $`\psi''+k^2\psi=0`$, то есть $`\psi=A\sin kx+B\cos kx`$. Условия на стенках дают
...
Возьмём $`U=0`$ при $`0\lt x\lt a`$, $`U=U_0`$ при $`x\gt a`$, слева — непроницаемая стенка. Связанное состояние имеет $`0\lt E\lt U_0`$. Решения, удовлетворяющие условиям на концах:


```math
\psi(x)=\begin{cases}
A\sin kx,&0\lt x\lt a,\\[0pt]
A\sin(ka)e^{-\kappa(x-a)},&x\gt a,
\end{cases}
\qquad
...
Для основного состояния $`\pi/2\lt z\lt \pi`$: функция имеет один максимум и не имеет внутренних узлов. При уменьшении $`U_0`$ связанный уровень подходит к порогу: $`E\to U_0`$, $`\kappa\to0`$, $`z\to\pi/2`$. Значит,
...
```math
\psi(x)=\begin{cases}\infty&\text{при }x\lt 0,\\kx&\text{при }x\gt 0.\end{cases}
```
...
**Краткая теория.** Параметр $`a\gt 0`$ задаёт степень локализации. Увеличение $`a`$ повышает кинетическую энергию и понижает среднюю потенциальную. Искомый компромисс находим вариационным методом.

**Решение.** Здесь $`k\gt 0`$ — наклон потенциала, а не волновое число. Для интегралов используем
...
Вторая производная $`\hbar^2/m+3k/a^3\gt 0`$, так что это минимум. В точке минимума $`\langle U\rangle=2\langle T\rangle`$, откуда
...
**Сопоставление с ответом книги.** Напечатанные там


```math
a_*=\left(\frac{3mk}{2\hbar^2}\right)^{1/3},\qquad
E_0\approx\frac32\left(\frac{9\hbar^2k^2}{4m}\right)^{1/3}
```


и оценка ошибки 6% полностью согласуются с полученным вариационным результатом.
...
```math
\psi(z)=A\begin{cases}\sin kz,&0\lt z\lt a,\\[0pt]
\sin(ka)e^{-\kappa(z-a)},&z\gt a.\end{cases}
```
...
**Условие.** Найти глубину ямы и энергию ионизации $`\mathcal E`$ электрона (в эВ), находящегося в основном состоянии в одномерной яме шириной $`a=2`$ Å с потенциалом $`U_0=\infty`$, $`U=-U_0`$ при $`0\lt x\lt a`$ и $`U=0`$ при $`x\gt a`$, если известно, что отношение волновой функции на границе ямы ($`x=a`$) к ее максимальному значению в яме равно $`\alpha=\sqrt3/2`$.
...
У основного состояния $`\pi/2\lt ka\lt \pi`$, так что максимум внутри равен $`A`$. Условие даёт $`\sin ka=\sqrt3/2`$. Корень $`ka=\pi/3`$ не подходит, поскольку имел бы положительную производную на выходе вместо отрицательной. Поэтому
...
Выбрана ветвь $`\pi/2\lt ka\lt \pi`$. Так как $`k^2=mU_0/\hbar^2`$,
...
Вторая производная положительна: $`3\pi^2\hbar^2/(m_eR^4)+8\pi\sigma\gt 0`$. Переводим поверхностное натяжение: $`1`$ дин/см $`=10^{-3}`$ Н/м, следовательно $`\sigma=3{,}5\cdot10^{-4}`$ Н/м.
