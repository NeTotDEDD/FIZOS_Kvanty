...
```math
\begin{aligned}
|1,1\rangle&=|\uparrow\uparrow\rangle,\\[0pt]
|1,0\rangle&=(|\uparrow\downarrow\rangle+|\downarrow\uparrow\rangle)/\sqrt2,\\[0pt]
|1,-1\rangle&=|\downarrow\downarrow\rangle,\\[0pt]
...
Сила направлена по движению пучка, осевой момент — по переносимому спину. В единицах книги $`F\simeq1{,}12\cdot10^4`$ дин, $`M\simeq3{,}3\cdot10^{-7}`$ дин·см. При поляризации степени $`P\lt 1`$ спиновый момент уменьшается в $`P`$ раз.
...
```math
E_1+E_2-2\mu B\lt 2E_1
\Rightarrow 2\mu B\gt E_2-E_1=\frac{3\pi^2\hbar^2}{2md^2}.
```
...
При $`B\gt B_c`$ основное состояние имеет магнитный момент $`2\mu`$ вдоль поля. При $`B=B_c`$ два состояния вырождены, поэтому равенство определяет порог, а не обязательную поляризацию. Этот вывод относится к основному состоянию; реальному переходу при изменении поля нужен механизм релаксации спина.
