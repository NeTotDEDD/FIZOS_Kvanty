...
```math
\begin{array}{ll}
E\gt U:&\psi=Ae^{ikx}+Be^{-ikx},\quad k=\sqrt{2m(E-U)}/\hbar;\\[2pt]
E\lt U:&\psi=Ae^{\kappa x}+Be^{-\kappa x},\quad \kappa=\sqrt{2m(U-E)}/\hbar;\\[2pt]
E=U:&\psi=A+Bx.
\end{array}
```
...
Для одной ступеньки при $`E\gt U_1,U_2`$ положим слева $`\psi=e^{ik_1x}+re^{-ik_1x}`$, справа $`\psi=te^{ik_2x}`$. Сшивка в нуле даёт
...
Пусть вне барьера $`U=0`$, внутри участка $`0\lt x\lt l`$ потенциал равен $`U_b`$. При $`E\gt U_b`$ обозначим $`k=\sqrt{2mE}/\hbar`$, $`q=\sqrt{2m(E-U_b)}/\hbar`$. Запишем справа $`\psi=t e^{ik(x-l)}`$. Тогда в точке $`l`$ значения равны $`t,ikt`$, а перенос решения к нулю даёт
...
```math
\boxed{D(E\gt U_b)=\left[1+\frac{U_b^2}{4E(E-U_b)}\sin^2(ql)\right]^{-1}.}
```


Эта формула применима и к яме: достаточно взять $`U_b\lt 0`$. Полная прозрачность возникает при $`ql=n\pi`$, то есть при целом числе полуволн внутри участка.

Для $`0\lt E\lt U_b`$ подставляем $`q=i\kappa`$, $`\sin(i\kappa l)=i\sinh(\kappa l)`$:


```math
\boxed{D(E\lt U_b)=\left[1+\frac{U_b^2}{4E(U_b-E)}\sinh^2(\kappa l)\right]^{-1}.}
```
...
```math
E_1=\frac{\pi^2}{4}\,13{,}606-2{,}5=31{,}07\ \text{эВ}\gt 0.
```
...
**Краткая теория.** В показателе ВКБ интегрируют $`\sqrt{U-E}`$ только там, где $`U\gt E`$. Отражение профиля слева направо не меняет этот интеграл.

**Решение.** Возьмём $`U(x)=U_0(1-x/(2a))`$ при $`0\lt x\lt 2a`$. Правая граница запрещённой области определяется из $`U(x_t)=E`$:
...
*Рис. 32 книги. Яма занимает $`0\lt x\lt a`$, барьер — $`a\lt x\lt 300a`$.*
...
У первого состояния внутри ямы нет узлов. Следовательно, нужно выбрать корень $`\pi/2\lt ka\lt \pi`$, а не прибавлять произвольное число $`\pi`$:
...
**Условие.** На одномерную прямоугольную потенциальную ступеньку высотой $`U_0\gt 0`$, расположенную в точке $`x=0`$, из области $`x\lt 0`$ падают микрочастицы с энергией $`\mathcal E=U_0/4`$ (рис. 37). На каком наименьшем расстоянии слева от ступеньки (в длинах волн де Бройля) плотность вероятности обнаружить частицу будет максимальна и на каком — минимальна?

![Потенциальная ступенька и стоячая волна перед ней](../assets/30_step_standing_wave.png)

**Краткая теория.** При $`\mathcal E\lt U_0`$ отражение полное, но его амплитуда имеет фазовый сдвиг. Именно этот сдвиг определяет положение узлов и пучностей интерференции падающей и отражённой волн.
...
```math
\psi(x)=\begin{cases}e^{ikx}+r e^{-ikx},&x\lt 0,\\ t e^{-\kappa x},&x\gt 0.\end{cases}
```
...
Нельзя ставить узел непосредственно на границе: такое условие применимо лишь к бесконечно высокой стенке. При конечной ступеньке волновая функция проникает в область $`x\gt 0`$.
...
**Условие.** На потенциальную ступеньку высотой $`U_0`$ падает поток микрочастиц с энергией $`\mathcal E\gt U_0`$. Найти давление частиц на ступеньку, если концентрация частиц равна $`n_0`$.
...
**Решение.** Считаем поток падающим нормально из области $`U=0`$ в область $`U=U_0\gt 0`$; $`n_0`$ — концентрация падающего пучка. Обозначим
