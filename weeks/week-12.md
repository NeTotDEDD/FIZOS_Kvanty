...
```math
\begin{aligned}
Q_{\beta^-}&=[M_a(A,Z)-M_a(A,Z+1)]c^2,\\
Q_{\beta^+}&=[M_a(A,Z)-M_a(A,Z-1)-2m_e]c^2,\\
...
**2. Разность энерговыделений.** Пусть $`B(A,Z)>0`$ — энергия связи. Для распада на альфа-частицу
...
```math
\begin{aligned}
\Delta Q={}&C_{\rm кул}\left[\frac{90^2-88^2}{226^{1/3}}-\frac{88^2-86^2}{222^{1/3}}\right]\\
...
```math
\begin{aligned}
G&=\frac{2\sqrt{2m}}{\hbar}\int_0^{C/\mathcal E}\sqrt{\frac Cr-\mathcal E}\,dr\\
