...
**Краткая теория.** Нужны законы сохранения для упругого рассеяния нейтрино на покоящемся электроне и условие $`\beta_e\gt 1/n`$.
...
Поскольку $`n^2-1\gt 0`$, условие $`\varepsilon\gt 0`$ даёт


```math
\cos\theta\gt \frac1{n\beta}.
```


Такой угол существует лишь при $`n\beta\gt 1`$, то есть $`v\gt c/n`$. Верхняя граница угла:
...
**Ответ:** энергия задаётся полученной формулой, а $`\boxed{\theta\lt \arccos[c/(nv)]}`$; равенство соответствует предельному случаю $`\varepsilon\to0`$.
