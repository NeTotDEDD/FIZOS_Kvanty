...
**Решение.** Условие $`W_{\rm ind}\gt W_{\rm sp}`$ означает


```math
\frac1{e^{h\nu/(k_BT)}-1}\gt 1
\Rightarrow e^{h\nu/(k_BT)}\lt 2
\Rightarrow\boxed{T\gt \frac{h\nu}{k_B\ln2}=\frac{hc}{\lambda k_B\ln2}.}
```


Одной численной температуры без длины волны получить нельзя. Для видимого диапазона $`\lambda\simeq400`$–$`700`$ нм порог меняется от $`5{,}19\cdot10^4`$ до $`2{,}97\cdot10^4`$ К; при $`\lambda=550`$ нм он равен $`3{,}77\cdot10^4`$ К. Для преобладания во всём этом диапазоне требуется $`T\gt 5{,}2\cdot10^4`$ К.
...
```math
\frac c{2L}\gt \frac{2\mu_BB}{h}
\Rightarrow L\lt \frac{hc}{4\mu_BB}.
```
...
**Условие.** Система, состоящая из атомов, имеющих два невырожденных уровня энергии $`E_1`$, и $`E_2\gt E_1`$, находится в тепловом равновесии. Выразить коэффициент поглощения света $`\kappa(T,\omega)`$ этой системой на частоте $`\omega=(E_2-E_1)/\hbar`$ через его значение $`\kappa_0`$ при $`T=0`$. Рассмотреть два предельных случая: 1) $`k_{\mathrm Б}T\gg\hbar\omega`$ и 2) $`k_{\mathrm Б}T\ll\hbar\omega`$.
